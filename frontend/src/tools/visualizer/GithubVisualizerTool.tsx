...
function parseGithubProfile(input: string): ProfileMeta | null {
  const normalized = input.trim().replace(/^(https?:\/\/)?(www\.)?github\.com\//i, '').replace(/\/$/, '');
...
function getExpertLevel(followers: number, publicRepos: number): { label: string; description: string; accent: string; badgeBg: string } {
...
function computeLanguageStats(repos: any[]): LanguageStat[] {
  ...
  repos.forEach(repo => {
    ...
    const existing = map.get(repo.language) ?? { 
...
function buildSvg(data: ProfileData, title: string, subtitle: string, ctaLabel: string): string {
  const safeTitle = title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeSubtitle = subtitle.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeCta = ctaLabel.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeName = data.name.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeLogin = data.login.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeBio = data.bio.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeLocation = data.location.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeCompany = data.company.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const safeWebsite = data.website.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

  const infoPanels: string[] = [];
  if (data.location) {
    infoPanels.push(`<g>
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  ...
  <g transform="translate(820, 520)">
    <rect x="0" y="0" width="260" height="70" rx="20" ry="20" fill="#2563eb" />
    <text x="130" y="45" fill="#ffffff" text-anchor="middle" font-family="Inter, sans-serif" font-size="22" font-weight="700">${safeCta}</text>
  </g>

  <text x="100" y="585" fill="#94a3b8" font-family="Inter, sans-serif" font-size="18">Generated with DevToolkit Visualizer</text>
</svg>`;
}

const GithubVisualizerTool: React.FC = () => {
  ...
  const [status, setStatus] = useState<string>('Ready to fetch GitHub profile metadata.');
  const [loading, setLoading] = useState(false);

  const profileMeta = useMemo(() => parseGithubProfile(profileInput), [profileInput]);
  const previewData: ProfileData = profileData ?? {
    login: profileMeta?.username ?? 'torvalds',
    name: profileMeta?.username ? profileMeta.username : 'Linus Torvalds',
  ...
  const expertLevel = useMemo(() => {
    return getExpertLevel(
      typeof previewData.followers === 'number' ? previewData.followers : 999,
      typeof previewData.publicRepos === 'number' ? previewData.publicRepos : 50
    );
  }, [previewData]);
  ...
  const fetchMetadata = async () => {
    ...
    setLoading(true);
    setStatus('Fetching GitHub profile and repository insights…');
    try {
      ...
      setStatus('Successfully loaded profile metadata & language analytics!');
    } catch (error) {
      setProfileData(null);
      setLanguageStats([]);
  ...
  const downloadSvg = () => {
    const svg = buildSvg(previewData, title, subtitle, ctaLabel);
  ...
  const copyMarkdown = async () => {
    const markdown = `![${previewData.name} on GitHub](data:image/svg+xml;base64,${btoa(buildSvg(previewData, title, subtitle, ctaLabel))})`;
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-100 p-4 sm:p-8 font-sans antialiased selection:bg-indigo-500 selection:text-white">
      <style>{`
        ...
        .animate-aura {
          animation: aura-glow 12s ease-in-out infinite alternate;
        }
        .animated-border {
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm sm:text-base mt-1 max-w-xl">
              Transform raw GitHub profiles into stunning interactive cards, README badges, and shareable social media banners.
            </p>
          </div>
        </header>
            ...
            <div className="glass-panel rounded-3xl p-6 shadow-2xl space-y-5">
              ...
              <div className="pt-2 space-y-3">
                <button
                  ...
                  disabled={loading}
                  className="w-full relative group overflow-hidden rounded-2xl p-[1px] font-semibold text-sm focus:outline-none"
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={downloadSvg}
                    className="flex items-center justify-center gap-2 rounded-2xl glass-card px-4 py-3 text-xs sm:text-sm font-semibold text-slate-200 hover:text-white hover:border-indigo-500/50 hover:bg-indigo-500/10 transition group"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-2xl glass-card px-4 py-3 text-xs sm:text-sm font-semibold text-slate-200 hover:text-white hover:border-pink-500/50 hover:bg-pink-500/10 transition group"
                  >
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="relative group float-card">
                ...
                <div className="relative rounded-[32px] bg-slate-950 p-6 sm:p-8 border border-slate-800 shadow-2xl space-y-6 overflow-hidden">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3.5 rounded-2xl glass-card text-center sm:text-left">
                      ...
                      <span className="text-xl sm:text-2xl font-black text-white mt-1 block">{previewData.followers}</span>
                    </div>
                    <div className="p-3.5 rounded-2xl glass-card text-center sm:text-left">
                      <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Following</span>
                      <span className="text-xl sm:text-2xl font-black text-white mt-1 block">{previewData.following}</span>
                    </div>
                    <div className="p-3.5 rounded-2xl glass-card text-center sm:text-left">
                      <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Public Repos</span>
                      <span className="text-xl sm:text-2xl font-black text-white mt-1 block">{previewData.publicRepos}</span>
                    </div>
                  </div>
                        ...
                        {languageStats.map((lang) => {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                width: `${pct}%`,
                                backgroundColor: lang.color || '#818cf8',
                              }}
                        ...
                        {languageStats.map((lang) => (
                          ...
                          >
                            <span
                              ...
                              style={{ backgroundColor: lang.color || '#818cf8' }}
                            />
